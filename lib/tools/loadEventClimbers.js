...
var argv = require('yargs')
    ...
    .usage("Usage: $0 [options] event-id csv-file\n" +
        "CSV format:\n" +
        "  Event Climber No., Member No., \"First Name\", \"Last Name\", \"Gender\", \"Category\", \"Region\", \"Team Name\"\n" + // xxx location, coach, birth_year
...
function loadClimbers(next) {
    ...
    csv()
        ...
        .on('record', function(row, index) {
            ...
            // Number, Member No.,"First Name","Last Name","Gender","Category","SCS Region","Team Name"
            climber = {
            ...
            if (row.length >= 5) {
                climber.gender = conv.convertToGender(row[4].trim());
            }
            if (row.length >= 6) {
                climber.category = conv.convertToCategory(row[5].trim());
            }
            if (row.length >= 7 && row[6]) {
                climber.region = conv.convertToRegion(row[6].trim());
            }
            if (row.length >= 8) {
                climber.team = row[7].match(/\*None.*/) ? null : row[7].trim();
            }
