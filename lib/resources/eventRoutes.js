...
function addResources(server, dbPool) {
    ...
    server.get("data/events/:eventId/routes", function(req,res,next) {
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            conn.query("SELECT id, version, routes_have_location, routes_have_color, num_rounds " +
                ...
                conn.query("SELECT number, color, location, points, sheet_row, sheet_column FROM event_route " +
                    ...
                    if (resultFormat === "csv") {
                        ...
                        columns = [
                            {label: "Number", prop: "number", width: 30},
                            {label: "Location", prop: "location", width: 50},
                        ...
                        try {
                            pdfu.printTable(doc, columns, event.routes, doc.x, doc.y, {
                                cellPaddingX: 10,
                                cellPaddingY: 6,
                                headerFontSize: 10,
                                cellFontSize: 10
