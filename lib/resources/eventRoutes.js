...
function printScoreCardPDF(res, event, clientRoot) {
    var i, columns, route, doc, x, y, pageWidth, colWidth, top, left,
        layout = event.scoreCardColumns > 2 ? "landscape" : "portrait",
        columnsRoutes = [];

    // xxx don't hardcode this!
    var logo1 = path.join(clientRoot, "upload", "Boston-Rock-Gym-logo-with-text.png");
    ...
    columns = [
        ...
        {label: "Points", prop: "points", width: 32},
        {label: "Falls", width: 40}, // blank so no prop
        {label: "Judges Initials", width: 55} //blank so no prop
    ];
    ...
    try {
        ...
        doc.text("Bib#: _______ Name: ___________________\nCategory: ______ Gender: ______", left + 8, top +8, {
            width: 320,
            height: 100
        });
        doc.rect(left, top, 336, 116).stroke();

        doc.image(logo1, left + colWidth + 50, top, { width: 100 } );
...
function addResources(server, dbPool, clientRoot) {
    ...
    server.get("data/events/:eventId/routes", function(req,res,next) {
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            conn.query("SELECT id, version, location, event_date, score_card_columns, routes_have_location, routes_have_color, num_rounds " +
