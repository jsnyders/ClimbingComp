...
function printScoreCardPDF(res, event, clientRoot) {
    var i, columns, route, doc, x, y, pageWidth, colWidth, top, left, tableHeight,
    ...
    // xxx don't hardcode this!
    var instructions = "Mark with a slash each and every fall.\nWhen a route is topped get the judges initials (if applicable) or the initials of two witnesses.";
    ...
    try {
        ...
        top = doc.y;
        doc.fontSize(10);
        doc.text("Bib#: _______\nName: ______________________\nCategory: _________ Gender: M / F\nBirth Year: ______", left + 8, top + 8, {
            width: 189 - 16,  // 2 5/8"
            height: 72 - 16   // 1"
        });
        doc.rect(left, top, 189, 72).stroke();
            ...
            conn.query("SELECT id, version, location, event_date, series, score_card_columns, routes_have_location, routes_have_color, num_rounds " +
                ...
                conn.query("SELECT number, color, location, points, sheet_row, sheet_column FROM event_route " +
                    ...
                    if (resultFormat === "csv") {
                        ...
                        columns.push("Route#");
                        if (event.routesHaveLocation) {
                            columns.push("Location"); // xxx should be configurable
                        }
                        if (event.routesHaveColor) {
                            columns.push("Color");
                        }
