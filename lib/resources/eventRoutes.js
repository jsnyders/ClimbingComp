...
function printScoreCardPDF(res, event, clientRoot) {
    ...
    tableHeight = 15 * (1 + columnsRoutes[0].length); // xxx 13 is based on table font and font size for header and cells
    ...
    try {
        ...
        if (logo1) {
            doc.image(logo1, pageWidth - 100, top, {width: 60});
        }

        if (logo2) {
            doc.image(logo2, pageWidth - 260, top, {width: 100});
        }
