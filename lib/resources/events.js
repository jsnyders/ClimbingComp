        ...
        dbPool.getConnection(function(err, conn) {
            ...
            conn.query("SELECT id, version, region, location, event_date, series, type, sanctioning, routes_have_location,routes_have_color,record_falls_per_climb " +
                ...
                event = {
                    ...
                    // xxx what about score card columns: scoreCardColumns: row.score_card_columns,
                    sanctioning: row.sanctioning,
                    recordFallsPerClimb: conv.fromDbBool(row.record_falls_per_climb),
                    routesHaveLocation: conv.fromDbBool(row.routes_have_location),
                    routesHaveColor: conv.fromDbBool(row.routes_have_color),
                ...
                conn.query("SELECT number, color, location, points, sheet_row, sheet_column FROM event_route WHERE event_id = ? ORDER BY sheet_column, sheet_row;", [eventId], function(err, rows) {
                    ...
                    // xxx consider copy climber info rather than reference
                    conn.query("SELECT number, ec.version, usac_member_id, first_name, last_name, gender, category, region, team, " +
