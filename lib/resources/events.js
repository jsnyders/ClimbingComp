...
function validateEventInput(input) {
    ...
    reason = validate.stringInSet("series", input.series, ["SCS", "ABS", "CCS", "Other"]); // todo generalize this
    if (reason) {
        return new InvalidInput(reason);
    }
...
function addResources(server, dbPool) {
    ...
    server.get("data/events/:eventId/data", function(req,res,next) {
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            conn.query("SELECT id, version, region, location, event_date, series, type, sanctioning, routes_have_location,routes_have_color,record_falls_per_climb " +
                ...
                event = {
                    ...
                    series: row.series,
                    type: row.type,
                    // xxx do we care about the state? This resource is for entering scores so probably assume state is open
                    // xxx what about scorecard columns: scoreCardColumns: row.score_card_columns,
                    sanctioning: row.sanctioning,
                    recordFallsPerClimb: conv.fromDbBool(row.record_falls_per_climb),
                    routesHaveLocation: conv.fromDbBool(row.routes_have_location),
                    routesHaveColor: conv.fromDbBool(row.routes_have_color),
                ...
                conn.query("SELECT number, color, location, points, sheet_row, sheet_column FROM event_route WHERE event_id = ? ORDER BY sheet_column, sheet_row;", [eventId], function(err, rows) {
                    ...
                    conn.query("SELECT ec.bib_number, ec.version, c.usac_member_id, c.first_name, c.last_name, c.gender, ec.category, ec.region, ec.team, " +
                            "total, top1, top2, top3, top4, top5, total_falls, climbs, ec.scored_on, ec.scored_by " +
                        ...
                        for (i = 0; i < rows.length; i++) {
                            ...
                            if (row.total_falls !== null || row.top1 !== null || row.climbs !== null) {
                                climber.scoreCard = {
                                    totalPoints: row.total, // xxx this should be calculated?
                                    top1: row.top1,
                                    top2: row.top2,
                                    top3: row.top3,
                                    top4: row.top4,
