...
function getEvent(conn, eventId, next) {
    conn.query("SELECT id, version, region, location, event_date, series, discipline, state, sanctioning, score_card_columns, " +
                    ...
                    "round_3_num_routes, round_3_format, round_3_num_advance" +
...
function makeEvent(input) {
    ...
    for (i = 0; i < input.rounds.length; i++) {
        baseKey = "round_" + (i + 1);
        row[baseKey + "_format"] = input[i].format;
        row[baseKey + "_num_routes"] = input[i].numRoutes;
        row[baseKey + "_num_advance"] = input[i].numAdvance;
    }
    return row;
}
...
function addResources(server, dbPool) {
    ...
    server.get("data/events", function(req,res,next) {
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            conn.query("SELECT id, region, location, event_date, series, sanctioning, state, updated_by, updated_on, version " +
                ...
                for (i = 0; i < rows.length; i++) {
                    ...
                    event = {
                        ...
                        location: row.location,
                        date: new Date(row.event_date),
                        series: row.series, // xxx is series or discipline more interesting or include both?
                        sanctioning: row.sanctioning,
                        state: row.state,
                        currentRound: row.current_round,
                        updatedBy: row.updated_by,
    ...
    server.get("data/events/:eventId/data", function(req,res,next) {
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            getEvent(conn, eventId, function(err, event) {
                ...
                event.routes = [];
                event.climbers = [];
                thisRound = event.rounds[event.currentRound];
