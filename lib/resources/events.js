...
//xxx        event_date: new Date(input.date), // xxx date conversion needs work
    ...
    reason = validate.stringInSet("format", input.type, ["Redpoint"]); // xxx todo support more types
    if (reason) {
        return new InvalidInput(reason);
    }
    */
    ...
    reason = validate.stringLength("scorecard location", input.scoreCardLocationLabel, 100);
    if (reason) {
        return new InvalidInput(reason);
    }
    reason = validate.stringLength("scorecard image 1", input.scoreCardImg1, 1000);
    if (reason) {
        return new InvalidInput(reason);
    }
    reason = validate.stringLength("scorecard image 2", input.scoreCardImg2, 1000);
    if (reason) {
        return new InvalidInput(reason);
    }
    reason = validate.stringLength("scorecard instructions", input.scoreCardInstructions, 1000);
    if (reason) {
        return new InvalidInput(reason);
    }
...
function makeEvent(input) {
    ...
    row =  {
        ...
        sc_location_label: input.scoreCardLocationLabel,
        sc_img_1: input.scoreCardImg1,
        sc_img_2: input.scoreCardImg2,
        sc_instructions: input.scoreCardInstructions,
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            getEvent(conn, eventId, function(err, event) {
                ...
                conn.query("SELECT number, color, location, points, sheet_row, sheet_column FROM event_route " +
                    ...
                    routeOffset += 1;
                    routes = "";
                    firstRoute = "route_" + routeOffset + "_score";
                    for (i = 0; i < thisRound.numRoutes; i++) {
                        routes += ", ec.route_" + (routeOffset + i) + "_score";
                    }
                    ...
                    conn.query(query, [eventId], function(err, rows) {
                        ...
                        for (i = 0; i < rows.length; i++) {
                            ...
                            if (row.total_falls !== null || row[firstRoute] !== null || row.climbs !== null) {
                                ...
                                for (j = 0; j < thisRound.numRoutes; j++) {
                                    // xxx consider rename top<n> to route<n>
                                    climber.scoreCard["top" + (j + 1)] = row["route_" + (routeOffset + j) + "_score"];
                                }
                            }
