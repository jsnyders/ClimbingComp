 ...
 * See also auth module which checks verifies the access token on requests.
...
function addResources(server, dbPool) {
    ...
    /*
     * Log In
     * After login return the value of authToken in AuthToken header of each request
     *
     * URI:data/sessions
     * Method: POST
    ...
    server.post("data/sessions", function(req,res,next) {
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            conn.query("SELECT username, role FROM app_user WHERE username = ? and pwd_hash = password(?);", [input.username, input.password], function(err, rows) {
                ...
                if (rows.length !== 1) {
                    conn.release();
                    console.log("xxx select rows len != 1");
