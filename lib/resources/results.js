...
//            "total_falls + if(top1 is null, 0, 1) + if(top2 is null, 0, 1) + if(top3 is null, 0, 1) /*xxx + if(top4 is null, 0, 1) + if(top5 is null, 0, 1)*/ as attempts, total 
    ...
    server.get("data/events/:eventId/results", function(req,res,next) {
        ...
        console.log("xxx get event results: event id: " + eventId + ", round: " + round + ", format: " + resultFormat);
        if (conv.isInvalid(eventId)) {
            return next(new restify.ResourceNotFoundError("Invalid event id"));
        }
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            conn.query("SELECT id, num_rounds, round_1_num_routes, round_2_num_routes, round_" + round + "_num_routes as num_routes, " +
                    "round_" + round + "_format as format, round_" + round + "_num_advance as num_advance " +
                    "FROM event WHERE id = ?;", [eventId], function (err, rows) {
                ...
                routes = "";
                total = ", ";
                attempts = ", total_falls, total_falls";
                ...
                console.log("xxx results query: " + query);

                conn.query(query, [eventId], function (err, rows) {
                    var i, j, breakIndex, climber, row, br, place, csvWriter, doc, columns, lastFalls, lastTotal, route,
                        report = [],
                        lastBreak = null;
                    ...
                    if (resultFormat === "csv") {
                        ...
                        res.header("Content-Disposition", "attachment; filename=\"" + event.location + "_" + formatDateForFilename(event.event_date) + ".csv\"");
