...
function addResources(server, dbPool) {
    ...
    var eventResultSQL =
        "SELECT ec.bib_number, c.usac_member_id, c.first_name, c.last_name, c.gender, ec.category, c.birth_date, ec.region, ec.team, scored_by, scored_on, " +
            ...
            // xxx why recalculate points it should be the same as total
            "(ifnull(top1, 0) + ifnull(top2, 0) + ifnull(top3, 0) + ifnull(top4, 0) + ifnull(top5, 0)) AS points, total " +
    ...
    server.get("data/events/:eventId/results", function(req,res,next) {
        ...
        dbPool.getConnection(function(err, conn) {
            ...
            query = conn.query(eventResultSQL, [eventId], function(err, rows) {
                var i, climber, row, br, place, csvWriter, doc, columns,
                    report = [],
                    lastBreak = null;
                ...
                if ( req.params.fmt === "export" ) {
                    ...
                    for (i = 0; i < rows.length; i++) {
                        ...
                        // xxx should only increment place if there isn't a tie
                        // todo implement correct rules for breaking a tie
                        if (row.total > 0) {
                            climber.place = place;
                            place += 1;
                        }
