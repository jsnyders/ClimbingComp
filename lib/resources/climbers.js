...
function insertClimber(conn, action, climber, next) {
    ...
    conn.query("SELECT id, usac_member_id, first_name, last_name, location, birth_date, birth_year, gender, category, region, team, coach " +
        ...
        if (rows.length !== 0) {
            ...
            if (climberChanges) {
                if (action === "update") {
                    conn.query("UPDATE climber SET ? WHERE id = ? AND usac_member_id = ?;",
                                [climberChanges, rows[0].id, climber.usac_memember_id], function (err, result) {
                        if (err) {
                            if (err.fatal) {
                                return next(err);
                            }
                    ...
                    next(null, "notUpdated", climberChanges.getOwnPropertyNames());
                }
            } else {
                next(null, "noChange");
            }
        } else {
            conn.query("INSERT INTO climber SET ?;", climber, function(err, result) {
                if (err) {
                    if (err.fatal) {
                        return next(err);
                    }
...
function addClimbers(conn, action, climbers, next) {
    ...
    function nextClimber() {
        if (index >= climbers.length) {
            ...
            insertClimber(conn, action, climber, function(err, result, reason) {
                if (err) {
...
function addResources(server, dbPool) {
    ...
    server.get("data/climbers", function (req, res, next) {
        ...
        dbPool.getConnection(function (err, conn) {
            ...
            conn.query("SELECT id, version, usac_member_id, first_name, last_name, location, gender, category, birth_year, birth_date, region, team, coach, updated_by, updated_on " +
            "FROM climber", function (err, rows) {
                var i, climber, row,
                    climbers = [];
     ...
     *      file CSV file
     *      hasHeader Boolean
     *      action one of "add", "replace", "update"
    ...
    server.post("data/climbers-upload", function(req,res,next) {
        ...
        action = input.action;
        reason = validate.stringInSet("action", action, ["add", "replace", "update"]);
        if (reason) {
        ...
        function validateClimberRow(climber, addError) {
            ...
            climber.gender = conv.convertToGender(climber.gender);
            if (conv.isInvalid(climber.gender)) {
                addError("Invalid gender");
            }
            climber.category = conv.convertToCategory(climber.category);
            if (conv.isInvalid(climber.category)) {
                addError("Invalid category");
            }
            climber.region = conv.convertToRegion(climber.region);
            if (conv.isInvalid(climber.category)) {
        ...
        csvLoader.loadFromCSV(csvFile, hasHeader, fields.length, fields, validateClimberRow, function(err, climbers) {
            ...
            if (err) {
                console.log("xxx error from csv ", err, climbers);
            ...
            dbPool.getConnection(function(err, conn) {
                ...
                addClimbers(conn, action, climbers, function(err, stats, errors) {
                    conn.release();
                    if (err) {
                        return next(err);
                    }
                    ...
                    if (errors.length === 0 && inputErrors.length === 0) {
                        ...
                        // concat and sort by line
                        errors.concat(inputErrors);
