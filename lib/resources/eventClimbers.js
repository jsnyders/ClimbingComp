...
var BIB_NUMBER_LENGTH = 3; // todo xxx this need to be configurable for now keep in sync with scorecard.js
...
function validateEventClimberInput(input) {
    ...
    if (conv.isInvalid(input.bibNumber) || input.bibNumber.toString().length !== BIB_NUMBER_LENGTH) {
...
function addResources(server, dbPool) {
    ...
    server.post("data/events/:eventId/climbers-upload", function(req,res,next) {
        ...
        function validateClimberRow(climber, addError) {
            ...
            if (conv.isInvalid(climber.bib_number) || climber.bib_number.toString().length !== BIB_NUMBER_LENGTH) {
