...
function importEventClimber(conn, action, climber, username, next) {
    ...
    // match rules:
    //   on usac_member_id is an exact match
    // or
    //   match on case independent compare of first AND last name (and one more factor?) if input has no member id
    //
    // perhaps it doesn't matter if the match is exact or not. All event climbers need an etnery in climber
    //
    // if exact match use climber info to fill in missing event climber info
    //   insert event_climber
    //    xxx update climber if diffs?
    // if no match and have gender and category add to climber table
    //   insert event climber
    //    xxx what if there is a member number in the input? It means they were not in the master list initially. trust it enough to add to climber?
