 ...
 * - Consider using cookies benefit of cookies is the don't require session storage and have some protection from JavaScript access.
...
module.exports = {
    ...
    authCheck: function(dbPool, sessionMaxTime, sessionIdleTime) {
        ...
        return function(req,res,next) {
            var token,
                headerToken = req.header("AccessToken"),
                cookies = cookie.parse(req.header("Cookie") || "");

            // if there is a header token it must match the cookie value
            if (headerToken) {
                if (cookies.ccst !== headerToken) {
                    req.log.error("Token did not match cookie.");
                    return next(new restify.InvalidCredentialsError());
                }
            }
            token = cookies.ccst || null;
...
//                            console.log("xxx idle time for session : " + row.idle_time + ", " + ttl);
                            ...
                            // else just not authenticated
                            return next(); // having a cookie that doesn't match a token isn't enough to return an error.
                        }
                    });
                });
            } else {
